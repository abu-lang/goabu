...
func TestLed2Buttons(t *testing.T) {
	...
	eLed, err := steel.NewExecuter(memLed, nil, communication.NewMemberlistAgent(8100, config.TestsLogConfig), config.TestsLogConfig)
	if err != nil {
		t.Fatal(err)
	}
	eLed.SetOptimisticExec(*steel.Optimistic)
	eLed.SetOptimisticInput(*steel.Optimistic)
	dummy, err := steel.NewExecuter(memButtons, []string{r1}, communication.NewMemberlistAgent(8101, config.TestsLogConfig, "127.0.0.1:8100"), config.TestsLogConfig)
	if err != nil {
		t.Fatal(err)
	}
	dummy.SetOptimisticExec(*steel.Optimistic)
	dummy.SetOptimisticInput(*steel.Optimistic)
...
func TestMotor(t *testing.T) {
	...
	e, err := steel.NewExecuter(mem, []string{r1, r2}, steel.MakeMockAgent(), config.TestsLogConfig)
	if err != nil {
		t.Fatal(err)
	}
